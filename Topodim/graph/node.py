...
class Node(ABC):
    ...
    def __init__(self, 
        ...
        self.query_predecessors: List[Node] = []
        self.query_successors: List[Node] = []
        self.debate_predecessors: List[Node] = []
        self.debate_successors: List[Node] = []
    ...
    def add_predecessor(self, operation: 'Node', st='spatial'):
        ...
        elif st == 'query' and operation not in self.query_predecessors:
            self.query_predecessors.append(operation)
            operation.query_successors.append(self)
        elif st == 'debate' and operation not in self.debate_predecessors:
            self.debate_predecessors.append(operation)
            operation.debate_successors.append(self)
    ...
    def add_successor(self, operation: 'Node', st='spatial'):
        ...
        elif st == 'query' and operation not in self.query_successors:
            self.query_successors.append(operation)
            operation.query_predecessors.append(self)
        elif st == 'debate' and operation not in self.debate_successors:
            self.debate_successors.append(operation)
            operation.debate_predecessors.append(self)
    ...
    def get_query_info(self, pre_node: any = None)->Dict[str,Dict]: # todo
        """ Return a dict that maps id to info. """
        query_info = {}
        if self.query_predecessors is not None:
            for predecessor in self.query_predecessors:
                if pre_node == predecessor.role:
                    ...
                    query_info[predecessor.id] = {"role":predecessor.role,"output":predecessor_output}
                else:
                    continue
        return query_info
    
    def get_evaluation_info(self, pre_node: any = None)->Dict[str,Dict]: # todo
        """ Return a dict that maps id to info. """
        query_info = {}
        if self.query_successors is not None:
            for successor in self.query_successors:
                if pre_node == successor.role:
                    successor_outputs = successor.outputs
                    if isinstance(successor_outputs, list) and len(successor_outputs):
                        successor_output = successor_outputs[-1]
                    elif isinstance(successor_outputs, list) and len(successor_outputs)==0:
                        continue
                    else:
                        successor_output = successor_outputs
                    query_info[successor.id] = {"role":successor.role,"output":successor_output}
                else:
                    continue
        return query_info
    
    def get_debate_info(self, pre_node: any = None)->Dict[str,Dict]: # todo
        """ Return a dict that maps id to info. """
        debate_info = {}
        if self.debate_successors is not None:
            for successors in self.debate_successors:
                print("pre node"+str(pre_node))
                if pre_node == successors.role:
                    print("debate good")
                    predecessor_outputs = successors.outputs
                    if isinstance(predecessor_outputs, list) and len(predecessor_outputs):
                        predecessor_output = predecessor_outputs[-1]
                    elif isinstance(predecessor_outputs, list) and len(predecessor_outputs)==0:
                        continue
                        ...
                        predecessor_output = predecessor_outputs
                    debate_info[successors.id] = {"role":successors.role,"output":predecessor_output}
                else:
                    continue
    ...
    async def async_execute(self, input:Any, mode:int = 0, pre_node: any = None, **kwargs):

        self.outputs = []
        if mode == 1:
            query_info:Dict[str,Any] = self.get_query_info(pre_node = pre_node)
            debate_info:Dict[str,Any] = {}
            evaluation_info:Dict[str,Any] = {}
        elif mode == 2:
            debate_info:Dict[str,Any] = self.get_debate_info(pre_node = pre_node)
            query_info:Dict[str,Any] = {}
            evaluation_info:Dict[str,Any] = {}
        elif mode == 3:
            evaluation_info:Dict[str,Any] = self.get_evaluation_info(pre_node = pre_node)
            query_info:Dict[str,Any] = {}
            debate_info:Dict[str,Any] = {}
        else:
            query_info:Dict[str,Any] = {}
        ...
        print("my name is :" + str(self.role) +", llm_name is :" + str(self.llm_name)+"\n\n")
        tasks = [asyncio.create_task(self._async_execute(input, spatial_info, temporal_info, query_info, debate_info, evaluation_info, **kwargs))]
    ...
    @abstractmethod
    async def _async_execute(self, input:List[Any], spatial_info:Dict[str,Any], temporal_info:Dict[str,Any], query_info:Dict[str,Any], debate_info:Dict[str,Any], evaluation_info:Dict[str,Any], **kwargs):
        """ To be overriden by the descendant class """
        """ Use the processed input to get the result """

    @abstractmethod
    def _process_inputs(self, raw_inputs:List[Any], spatial_info:Dict[str,Any], temporal_info:Dict[str,Any], query_info:Dict[str,Any], debate_info:Dict[str,Any], evaluation_info:Dict[str,Any], **kwargs)->List[Any]:
        """ To be overriden by the descendant class """
